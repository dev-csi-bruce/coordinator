...
class CoordinatorAdminService(
    ...
    fun addProduct(product: ProductDTO): AddProductDTO.Response {
        if (product.id != null && product.id < 0) throw CoordinatorServerException(INVALID_PRODUCT_ID)

        val brand = brandRepository.findByName(product.brandName) ?: throw CoordinatorServerException(INVALID_BRAND_NAME)
        val category = categoryRepository.findByName(product.categoryName) ?: throw CoordinatorServerException(INVALID_CATEGORY_NAME)
    ...
    fun addBrand(brand: BrandDTO): AddBrandDTO.Response {
        if (brand.id != null && brand.id < 0) throw CoordinatorServerException(INVALID_BRAND_ID)
    ...
    fun updateProduct(product: ProductDTO): UpdateProductDTO.Response {
        ...
        val targetProduct = productRepository.findByIdOrNull(product.id!!) ?: throw CoordinatorServerException(INVALID_PRODUCT_ID)
        val brand = brandRepository.findByName(product.brandName) ?: throw CoordinatorServerException(INVALID_BRAND_NAME)
        val category = categoryRepository.findByName(product.categoryName) ?: throw CoordinatorServerException(INVALID_CATEGORY_NAME)
    ...
    fun deleteProduct(id: Int): DeleteProductDTO.Response {
        val targetProduct = productRepository.findByIdOrNull(id) ?: throw CoordinatorServerException(INVALID_PRODUCT_ID)
    ...
    fun updateBrand(brand: BrandDTO): UpdateBrandDTO.Response {
        val targetBrand = brandRepository.findByIdOrNull(brand.id!!) ?: throw CoordinatorServerException(INVALID_BRAND_ID)
    ...
    fun deleteBrand(id: Int): DeleteBrandDTO.Response {
        val targetBrand = brandRepository.findByIdOrNull(id) ?: throw CoordinatorServerException(INVALID_BRAND_ID)
